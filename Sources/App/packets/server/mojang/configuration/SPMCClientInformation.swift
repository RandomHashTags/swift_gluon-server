...
public extension ServerPacketMojang.Configuration {
    ...
    struct ClientInformation : ServerPacketMojangConfigurationProtocol {
        public static let id:ServerPacketMojangConfiguration = ServerPacketMojangConfiguration.client_information
        
        public static func parse(_ packet: GeneralPacketMojang) throws -> Self {
            let locale:String = try packet.read_string()
            let view_distance:Int8 = try packet.read_byte()
            let chat_mode:ChatMode = try packet.read_enum()
            let chat_colors:Bool = try packet.read_bool()
            let displayed_skin_parts:UInt8 = try packet.read_unsigned_byte()
            let main_hand:MainHand = try packet.read_enum()
