...
public struct BlockPopulator : Jsonable {
    ...
    static func populate(chunk: Chunk) {
        ...
        for x in 0..<16 {
            let x_double:Double = Double(x)
            let location_x:Double = Double(chunk.x) * 16 + x_double
            let location_x_string:String = location_x.description
            for z in 0..<16 {
                let z_double:Double = Double(z)
                let location_z:Double = Double(chunk.z) * 16 + z_double
